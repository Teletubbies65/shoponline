...
<main>
  <section class="py-5 text-center container">
    <div class="album py-5" style="background-color: #d3d3d3;">
		...
		<img class="mb-4" src="../album/00.jpg" width="150" height="150" <br>
        <p>
...
<form method="post" action="" enctype="multipart/form-data" align = "center">
	ชื่อสินค้า <input type="text" name="fname" required autofocus><br>
    รายละเอียดสินค้า<br>
	<textarea name="fdetail" rows="5" cols="50"></textarea><br>
    ราคา <input type="text" name="fprice" required><br>
...
if (isset($_POST['Submit'])) {
    ...
    if ($_FILES['fimg']['error'] === UPLOAD_ERR_OK) {
        ...
        $detail = mysqli_real_escape_string($conn, $_POST['fdetail']);
        $price = mysqli_real_escape_string($conn, $_POST['fprice']);
        $cat_id = mysqli_real_escape_string($conn, $_POST['fcat']);

        $sql = "INSERT INTO `food` (`f_name`, `f_detail`, `f_price`, `f_ext`, `c_id`) 
                VALUES ('$name', '$detail', '$price', '$ext', '$cat_id')";
        mysqli_query($conn, $sql) or die("เพิ่มข้อมูลสินค้าไม่ได้");
        $idauto = mysqli_insert_id($conn);

        move_uploaded_file($_FILES['fimg']['tmp_name'], "images/" . $idauto . "." . $ext);
        ...
        echo "<script>alert('มีข้อผิดพลาดในการอัพโหลดไฟล์');</script>";
    }
}
