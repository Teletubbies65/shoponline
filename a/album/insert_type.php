...
<main>
  <section class="py-5 text-center container">
   ...
   <form method="post" action="insert_type.php" enctype="multipart/form-data"  align = "center">
    ประเภทสินค้า: <br>
    <textarea name="cname" rows="4" cols="100" required></textarea>
    <br>
...
if (isset($_POST['Submit'])) {
    ...
    // ตรวจสอบว่าได้รับค่า cid
    if (isset($_POST['cname']) && !empty($_POST['cname'])) {
        $cid = mysqli_real_escape_string($conn, $_POST['cname']);
        
        $sql = "INSERT INTO `category` (`c_name`, `c_id`) 
                VALUES ('$name', '$cid')";
        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('เพิ่มข้อมูลสินค้าสำเร็จ'); window.location='type.php';</script>";
        } else {
            echo "<script>alert('เพิ่มข้อมูลไม่สำเร็จ: " . mysqli_error($conn) . "');</script>";
        }
    } else {
        echo "<script>alert('กรุณาเลือกประเภทสินค้า');</script>";
    }
}
